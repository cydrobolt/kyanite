...
if (!is_array($auth->islogged())) {
    ...
    //if attempting to change settings
    
    if($_POST['setchg']=='setchg') {
        if(is_string($_POST['cpw'])&&is_string($_POST['npw'])){
        $cpw = $mysqli->real_escape_string($_POST['cpw']);
        $npw = $mysqli->real_escape_string($_POST['npw']);
        }
        if(is_string($_POST['theme'])) {
            $theme = $mysqli->real_escape_string($_POST['theme']);
            $themes = array('flatly','darkly','yeti','cosmo','simplex');
            ...
            if($thm!=1) {
                echo "<h2>Invalid Settings Specified. Try again.</h2>";require_once('footer.php');die();
            }
    ...
    echo "<h3>Polr Dashboard</h3><br>".$addldata;
  ...
  <br><br><input type="submit" class="btn-success btn-lg" disabled="disabled"/>
    ...
    if ($isadmin == true) {
        ...
        echo '<div class="tab-pane" id="adminpanel"><br>Polr Links - Limited @ 360:'.$linksadmin.'<br>Polr Users - Limited @ 360:'.$usersadmin.'<br>Debug Variables: <br>Default IP Fetch: '.$ip.'<br>X-Forwarded-For:'.@$headers['X-Forwarded-For'].'<br>Forwarded-For'.@$headers['forwarded-for'];//<br><br>Session Dump:'.$sessiondump.'</div>';
    }
